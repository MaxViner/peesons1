...
public class View {
    ...
    private static String getChar(int x, int y){
        String out = "| ";
        for (Person active: Main.allTeam) {
            if (active.x == x && active.y == y){
                if (active.hit_points <= 0) {
                    out = "|" + (AnsiColors.ANSI_RED +active.name_of_class+" "+ active.name + AnsiColors.ANSI_RESET);
                    break;
                }

                if (Main.team2.contains(active)) out = "|" + (AnsiColors.ANSI_GREEN +active.name_of_class+" "+ active.name+ active.hit_points + AnsiColors.ANSI_RESET);
                if (Main.team1.contains(active)) out = "|" + (AnsiColors.ANSI_BLUE +active.name_of_class+" "+ active.name+active.hit_points + AnsiColors.ANSI_RESET);
                break;
            }
        }
        return out;
    ...
    public static void view() {
        ...
        for (int i = 0; i < l[0]*2; i++) System.out.print("_______");
        System.out.println("");
